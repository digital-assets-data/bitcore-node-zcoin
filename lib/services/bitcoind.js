...
Bitcoin.prototype.getAPIMethods = function () {
    var methods = [
        ...
        ['getTotalSupply', this, this.getTotalSupply, 0]
    ];
    return methods;
};
...
/**
 * Called by Node to determine the available API methods.
 * No CamelCase for Zcoin!

 Bitcoin.prototype.getAPIMethods = function() {
  var methods = [
    // commented out api methods are not supported by ZCoin
    ['getblock', this, this.getBlock, 1],
    // ['getrawblock', this, this.getRawBlock, 1],
    // ['getBlockHeader', this, this.getBlockHeader, 1],
    // ['getBlockOverview', this, this.getBlockOverview, 1],
...
Bitcoin.prototype._loadSpawnConfiguration = function (node) {
    ...
    var configPath = (spawnOptions['configPath']) ? path.resolve(spawnOptions.configPath) : path.resolve(spawnOptions.datadir, './zcoin.conf');
...
Bitcoin.prototype._checkConfigIndexes = function (spawnConfig, node) {
    ...
    $.checkState(
        ...
        '"zmqpubhashblock" option is required to get event updates from zcoind. ' +
    ...
    if (spawnConfig.reindex && spawnConfig.reindex === 1) {
        log.warn('Reindex option is currently enabled. This means that zcoind is undergoing a reindex. ' +
            ...
            'of bitcore-node-zcoin services will start.');
        node._reindex = true;
    }
};
...
Bitcoin.prototype._getNetworkConfigPath = function () {
    var networkPath;
    if (this.node.network === bitcore.Networks.testnet) {
        networkPath = 'testnet3/zcoin.conf';
        if (this.node.network.regtestEnabled) {
            networkPath = 'regtest/zcoin.conf';
        }
    }
    return networkPath;
};
        ...
        if (!self.node.stopping) {
            self.syncPercentage(function (err, percentage) {
                if (err) {
                    ...
                    log.info('Zcoin Height:', self.height, 'Percentage:', percentage.toFixed(2));
                }
            });
        }
    }
    ...
    if (node._reindex) {
        interval = setInterval(function () {
            node.client.getBlockchainInfo(function (err, response) {
                ...
                log.info('Zcoin Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
...
Bitcoin.prototype._stopSpawnedBitcoin = function (callback) {
    var self = this;
    var spawnOptions = this.options.spawn;
    var pidPath = spawnOptions.datadir + '/zcoind.pid';
    ...
    function stopProcess() {
        fs.readFile(pidPath, 'utf8', function (err, pid) {
            ...
            try {
                log.warn('Stopping existing spawned zcoin process with pid: ' + pid);
                self._process.kill(pid, 'SIGINT');
            } catch (err) {
                if (err && err.code === 'ESRCH') {
                    log.warn('Unclean zcoin process shutdown, process not found with pid: ' + pid);
                    return callback(null);
                } else if (err) {
                    return callback(err);
                }
...
Bitcoin.prototype._spawnChildProcess = function (callback) {
    ...
    self._stopSpawnedBitcoin(function (err) {
        ...
        log.info('Starting zcoin process');
        ...
        self.spawn.process.once('exit', function (code) {
            if (!self.node.stopping) {
                log.warn('Zcoin process unexpectedly exited with code:', code);
                log.warn('Restarting zcoin child process in ' + self.spawnRestartTime + 'ms');
                setTimeout(function () {
                    self._spawnChildProcess(function (err) {
                        if (err) {
                            return self.emit('error', err);
                        }
                        log.warn('Zcoin process restarted');
                    });
        ...
        async.retry({times: 60, interval: self.startRetryInterval}, function (done) {
            ...
            if (exitShutdown) {
                return callback(new Error('Stopping while trying to spawn zcoind.'));
            }
...
Bitcoin.prototype._connectProcess = function (config, callback) {
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function (done) {
        ...
        if (exitShutdown) {
            return callback(new Error('Stopping while trying to connect to zcoind.'));
        }
...
Bitcoin.prototype._getTxidsFromMempool = function (deltas, callback) {
    var txidMap = new Map();
    var self = this;

    for (var i = 0; i < deltas.length; i++) {
      txidMap.set(deltas[i].txid, 0);
    }
    ...
    txidMap.forEach(function(value, txid) {
       txidVec.push(txid);
    });

    async.map(txidVec, function (txid, next) {
      ...
      for(var i=0; i < detailed.length; ++i) {
          if(detailed[i] != null
            && typeof detailed[i] === 'object'
            && detailed[i].hasOwnProperty('height')) {
              if(detailed[i].height == -1) {
                mempoolTxids.push({txid: detailed[i].hash, value: detailed[i].inputSatoshis});
              }
          }
      }
      callback(null, mempoolTxids);
    });
};
...
Bitcoin.prototype.getAddressTxids = function (addressArg, options, callback) {
    ...
    if (queryMempool) {
        self.client.getAddressMempool({addresses: addresses}, function (err, response) {
            ...
            self._getTxidsFromMempool(response.result, function (err, results){
                for(var i =0; i < results.length; ++i) {
                  mempoolTxids.push(results[i].txid);
                }
                finish();
            });
        });
 ...
 * @param {Object} txid - A zcoin transaction id
 * @param {Function} callback
 */
    ...
    function querySummary() {
        async.parallel([
            ...
            function getMempool(done) {
                ...
                if(updateMempool) {
                    summary = self.summaryCache.get(cacheKey);
                }
                self.client.getAddressMempool({'addresses': addresses}, function (err, response) {
                    if (err) {
                        return done(self._wrapRPCError(err));
                    }
                    self._getTxidsFromMempool(response.result, function(err, result){
                      summary.unconfirmedAppearances = result.length;
                      var balance = 0;
                      for(var i=0; i < result.length; ++i) {
                        balance += result[i].value;
                      }
                      summary.unconfirmedBalance = -balance;
                      done();
                    });
                });
            },
...
Bitcoin.prototype.getDetailedTransaction = function (txid, callback) {
    ...
    function addInputsToTx(tx, result) {
        ...
        for (var inputIndex = 0; inputIndex < result.vin.length; inputIndex++) {
            ...
            /*
            "in": {
                "txid": "617c3f8e233e7305cc6b1088dc529ce616bc2fe1600585dc9f186168d65f6e09",
                "vout": 0,
                "scriptSig": {
                    "asm": "304402205f47b14e29046af50c32cbb4708926d479e72fc8394a6afe5a34d5bc567b5f6c0220447630fb16ce1003ce30c918ce9ef8cdfc89e5db9702235acd781f5429e0449c[ALL] 028a428b7d6a72e1f184baf2ad5e9de4a449cc1eae36129b6f18f2f52201660a91",
                    "hex": "47304402205f47b14e29046af50c32cbb4708926d479e72fc8394a6afe5a34d5bc567b5f6c0220447630fb16ce1003ce30c918ce9ef8cdfc89e5db9702235acd781f5429e0449c0121028a428b7d6a72e1f184baf2ad5e9de4a449cc1eae36129b6f18f2f52201660a91"
                },
                "sequence": 4294967294
            }
            */
    ...
    function addOutputsToTx(tx, result) {
        ...
        for (var outputIndex = 0; outputIndex < result.vout.length; outputIndex++) {
            ...
            "out": {
                ...
                "n": 1,
                "scriptPubKey": {
                    "asm": "OP_DUP OP_HASH160 9c2cb741c1773926cf0f0969286b9a9a337ecd99 OP_EQUALVERIFY OP_CHECKSIG",
                    "hex": "76a9149c2cb741c1773926cf0f0969286b9a9a337ecd9988ac",
                    "reqSigs": 1,
                    "type": "pubkeyhash",
                    "addresses": [
                        "aExEijzXcsTKjDgpjJ6VdSU2kHH5Q81oxV"
                    ]
                }
            }
            */
            out.valueSat = utils.safeMul(out.value,1e8);
    ...
    if (this.spawn && this.spawn.process) {
        ...
        this.spawn.process.once('exit', function (code) {
            if (!exited) {
                exited = true;
                if (code !== 0) {
                    var error = new Error('zcoind spawned process exited with status code: ' + code);
                    error.code = code;
        ...
        setTimeout(function () {
            if (!exited) {
                exited = true;
                return callback(new Error('zcoind process did not exit'));
            }
...
/**
 * Returns the sum of coins of coinbase txes
 * @param {Function} callback
 */
Bitcoin.prototype.getTotalSupply = function (callback) {
  var self = this;
  console.log('getTotalSupply:');
  this.client.getTotalSupply(function (err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
    }
    var result = response.result;
